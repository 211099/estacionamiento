...
package com.example.estacionamientoc2.models;
...
public class Vehiculo extends Observable implements Runnable {
    ...
    public void run() {
        try {
            //Se encarga de controlar el acceso de entrada y salida
            mutex.acquire();
            ...
            mutex.acquire();
            if (e.eSalida > 0 || e.autodentro) {//verifica si hay vehiculos en salida
                e.eEntrada++;
            ...
            e.autodentro = false;
            if (e.eEntrada > 0) {//permite que entre otro carro
                puertaE.release();
            } else if (e.eSalida > 0) {
                puertaS.release();
            } else {
            ...
            mutex.acquire();
            for (Vehiculo v : e.caja) {//busca un lugar para estacionarse
            ...
            if (!aux) {//verifica que haya vehiculos  agregados
                e.caja.add(this);//Agerga vehiculos
            ...
            mutex.acquire();
            if (e.eEntrada > 0 || e.autodentro) {// verifica cuantos carros hay
                e.eSalida++;
            ...
            e.autodentro = false;
            if (e.eSalida > 0) {//Verifica las salidas de los carros
                puertaS.release();
